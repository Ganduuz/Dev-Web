...
async function handleCreate(e) {
  ...
  try {
    ...
    // ✅ On récupère l'_id AVANT de fermer le modal
    const factureId = newFacture.data._id;
    ...
    setMsg("✅ Facture créée! Génération du PDF...");

    load();

    // ✅ On génère le PDF avec l'_id déjà récupéré, pas de setTimeout risqué
    await handleGeneratePDF(factureId);

  } catch (err) {
    alert(err.response?.data?.error || "Erreur création facture");
  }
}
...
async function handleDownloadPDF(factureId, reference) {
  try {
    ...
    // ✅ Vérification que la réponse est bien un PDF et pas un JSON d'erreur
    if (response.data.type === "application/json") {
      const text = await response.data.text();
      const err = JSON.parse(text);
      setMsg(`❌ ${err.error || "PDF non disponible"}`);
      return;
    }
    ...
    setMsg("❌ Erreur lors du téléchargement du PDF");
...
async function handleGenerateFromMission(mission) {
  try {
    ...
    // ✅ Fix [object Object]
    const msg = err.response?.data?.error || err.response?.data || err.message || "Erreur";
    if (err.response?.status === 409) {
      setMsg("⚠️ Une facture existe déjà pour cette mission");
    } else {
      setMsg(`❌ ${msg}`);
    }
    setTimeout(() => setMsg(""), 4000);
...
async function handleCreate(e) {
  ...
  try {
    ...
    const factureId = newFacture.data._id; // ✅ récupéré avant tout
    setShowModal(false);
    setForm({ missionId: "", clientNom: "", clientAdresse: "", montant: "", tva: 20, notes: "", dateEcheance: "" });
    setAnalysisResult(null);
    setMsg("✅ Facture créée ! Génération du PDF...");
    load();
    await handleGeneratePDF(factureId); // ✅ plus de setTimeout
    setTimeout(() => setMsg(""), 4000);

  } catch (err) {
    const errMsg = err.response?.data?.error || err.response?.data || err.message || "Erreur";
    setMsg(`❌ ${errMsg}`);
    setTimeout(() => setMsg(""), 4000);
  }
}
